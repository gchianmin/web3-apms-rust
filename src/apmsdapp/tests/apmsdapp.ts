...
describe("apmsdapp", async () => {
  ...
  const assignReviewer =async (conferenceId, paperId, reviewers) => {
    await program.rpc.assignReviewer(
      conferenceId, paperId, reviewers,
      {
        accounts: {
          conferenceList: conferencePDA,
          user: user.publicKey,
  ...
  const assignChair =async (conferenceId, paperId, chair) => {
    await program.rpc.assignChair(
      conferenceId, paperId, chair,
      {
        accounts: {
          conferenceList: conferencePDA,
          user: user.publicKey,
  ...
  it("Assigning a reviewer", async () => {
    ...
    await assignReviewer(id, paperId, reviewers)
  ...
  it("Assigning a chair", async () => {
    ...
    await assignChair(id, paperId, chair)

    const updatedData = await program.account.conferenceListAccountData.fetch(conferencePDA);
    console.log("sub", updatedData.conferences[0].paperSubmitted[0])
    assert.equal(updatedData.count, 1);
    assert.equal(updatedData.conferences.length, 1);
    assert.equal(updatedData.deletedIndexes.length, 1);
    assert.equal(updatedData.conferences[0].paperSubmitted.length, 1);
